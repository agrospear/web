  ...
  return [
    {
      ...
      department: {
        ...
        description: 'Agrochemical manufacturing division of Qingdao Vatrad Group Co., Ltd.',
        sameAs: BRAND_PARENT_URL,
      },
      ...
      hasCredential: [
        ...
        { '@type': 'EducationalOccupationalCredential', credentialCategory: 'certification', name: 'ICAMA' },
        { '@type': 'EducationalOccupationalCredential', credentialCategory: 'certification', name: 'BSCI' },
      ],
      knowsAbout: [
        'Agrochemical manufacturing',
        'agrochemical formulation plant',
        'Agrochemical OEM / ODM',
        'custom agrochemical formulations',
        'Agrochemical product development',
        'Formulation pilot trials',
...
export function serviceLd(input: {
  ...
  return {
    ...
    audience: {
      ...
      name: 'Distributors, importers, agrochemical brands and organizations',
    },
  }
}
...
export function brandHeritageLd(): Record<string, unknown> {
  return {
    ...
    historyHighlights: [
      { '@type': 'Event', name: 'Founding', startDate: '2012', description: `Founded as the agrochemical manufacturing division of ${BRAND_COMPANY_NAME}.` },
      { '@type': 'Event', name: 'Plant operations', description: `${FACTS.warehouseM2} agrochemical formulation plant with ${FACTS.workshops} in Qingdao, China.` },
      ...
      { '@type': 'Event', name: 'Global exports', description: `Supplies agrochemical OEM/ODM partners in ${FACTS.exportCountries} countries worldwide.` },
    ],
...
export function enhancedFaqLd(): Record<string, unknown> {
  const faqs: {
  ...
  }[] = [
    {
      ...
      category: 'General',
      priority: 5,
      keywords: [SITE_NAME, 'Agrochemical manufacturer', 'Agrochemical formulation plant', 'Qingdao'],
    },
    {
      q: 'What is the MOQ for custom agrochemical orders?',
      a: `Trial/pilot orders start at ${MOQ_SHORT.trialStandard}; standard production runs are ${MOQ_SHORT.standardRun}.`,
      category: 'Pricing',
      priority: 5,
      keywords: ['MOQ', 'minimum order quantity', 'pilot batch', 'custom agrochemical'],
    },
    {
      q: 'What are the production and sampling lead times?',
      a: `Samples ship in ${FACTS.sampleTime}; bulk production is ${FACTS.leadTime} from confirmed PO and deposit. Custom formulation development adds 15\u201320 days.`,
      category: 'Production',
      priority: 5,
      keywords: ['lead time', 'sample time', 'production time', 'bulk order'],
    },
    {
      q: 'Can you manufacture agrochemical formulations under my own brand?',
      a: 'Yes \u2014 OEM and private-label manufacturing: formulation development, sampling, production and export. You own the brand, the market and the customer.',
      category: 'Customization',
      priority: 5,
      keywords: ['private label', 'OEM', 'ODM', 'custom formulation', 'own brand'],
    },
    {
      q: 'What quality control do you run on every batch?',
      a: `Every batch passes ${FACTS.qualityGates} inspection gates including a ${FACTS.qcChecklistPoints} production checklist and ${FACTS.qualityTest}. Batches keep ${FACTS.traceabilityRet} ERP traceability, and third-party inspections by ${FACTS.thirdPartyInspectors.join(', ')} are available.`,
      category: 'Technical',
      priority: 4,
    ...
    },
    {
      ...
      category: 'Technical',
      priority: 4,
      keywords: ['certifications', 'ISO 9001', 'ISO 14001', 'ICAMA', 'BSCI', 'FAO/WHO'],
    },
    {
      q: 'What warranty do you provide on bulk orders?',
      a: 'A 5-year limited warranty covers active ingredient content, formulation stability, packaging integrity, labeling accuracy and specification conformity; non-standard storage conditions carry a 1-year warranty. Warranty terms are written into each order contract.',
      category: 'Warranty',
      priority: 5,
...
export function warrantyReturnsLd(): Record<string, unknown> {
  return {
    ...
    description:
      '5-year limited warranty on active ingredient content, formulation stability, packaging integrity, labeling accuracy and specification conformity. Non-standard storage conditions reduce coverage to 1 year.',
    coverage: [
      { '@type': 'Thing', name: 'Active ingredient content', warranty: '5 years' },
      { '@type': 'Thing', name: 'Formulation stability', warranty: '5 years' },
      { '@type': 'Thing', name: 'Packaging integrity', warranty: '5 years' },
      { '@type': 'Thing', name: 'Labeling accuracy', warranty: '5 years' },
      { '@type': 'Thing', name: 'Specification conformity', warranty: '5 years' },
      { '@type': 'Thing', name: 'Non-standard storage conditions', warranty: '1 year' },
    ],
    claimProcess: {
      '@type': 'HowTo',
      name: 'Warranty claim',
      ...
      step: [
        ...
        { '@type': 'HowToStep', position: 3, name: 'Replace or reformulate', text: 'Defective batches are replaced or reformulated at our option, per the warranty terms in the order contract.' },
      ],
    },
  }
}
...
export function shippingLogisticsLd(): Record<string, unknown> {
  return {
    ...
    shippingMethods: [
      { '@type': 'OfferShippingDetails', name: 'Sea freight', description: 'Bulk production shipments (LCL/FCL) \u2014 standard for 20+ ton production runs.' },
  ...
  return {
    ...
    mainEntity: [
      ...
      },
      {
        ...
        acceptedAnswer: {
          '@type': 'Answer',
          text: 'Yes \u2014 formulation, labeling and packaging are customized per PO within the standard MOQ tiers.',
        },
      },
      {
        '@type': 'Question',
        ...
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Samples in ${FACTS.sampleTime}; bulk production ${FACTS.leadTime} after PO confirmation. Custom formulation development adds 15\u201320 days.`,
        },
      },
    ],
  }
...
export function qcHowToLd(): Record<string, unknown> {
  return {
    '@context': 'https://schema.org',
    '@type': 'HowTo',
    name: '7-Stage Agrochemical Quality Control Inspection',
    description: 'Every agrochemical formulation batch passes seven inspection gates with documented pass/fail criteria before shipment. Each gate has quantitative standards and traceability records.',
    step: [
      { '@type': 'HowToStep', position: 1, name: 'Incoming Material Inspection', text: 'Active ingredients, co-formulants and all raw materials inspected against specification before entering production. Batch certificates and ICAMA/FAO specifications verified.' },
      { '@type': 'HowToStep', position: 2, name: 'Formulation Preparation Inspection', text: 'Active ingredient concentration and co-formulant ratios verified. Purity and assay values checked against specification (SC, EC, WP, WDG, SL, ME, CS, FS formulation types).' },
      { '@type': 'HowToStep', position: 3, name: 'Mixing & Processing', text: 'Formulation processing parameters (temperature, mixing speed, dwell time) logged per production run. Homogeneity and stability inspected with documented reject rates.' },
      { '@type': 'HowToStep', position: 4, name: 'Stability & Performance Testing', text: 'Formulation stability and performance hold test per FAO/WHO specification. Auto-reject above specification deviation limits. Test logs retained per batch.' },
      { '@type': 'HowToStep', position: 5, name: 'Packaging & Labeling', text: 'Container integrity, label accuracy, dosage instructions and all packaging components checked against frozen BOM specification.' },
      { '@type': 'HowToStep', position: 6, name: 'Graphic & Branding QC', text: 'Product labeling, logo placement and color accuracy verified against approved artwork. Brand application inspected per client specification.' },
      { '@type': 'HowToStep', position: 7, name: 'Final Packaging & Documentation', text: 'Export packaging, compliance documentation (ICAMA, ISO 9001, ISO 14001, BSCI, FAO/WHO), certificates of conformity and batch traceability records verified before shipment.' },
    ],
  }
}
