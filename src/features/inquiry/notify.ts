...
export async function sendInquiryNotification(apiKey: string | null, from: string, to: string[], data: InquiryNotifyData): Promise<void> {
  ...
  const text = [
    ...
    `Role: ${i.role || '—'} · Formulation platform: ${i.formulationPlatform || '—'}`,
    `Formulation type: ${i.formulationType || '—'} · Packaging: ${i.packaging || '—'}`,
  ...
  const html = [
    ...
    row('Role / formulation platform', `${e(i.role)}${i.formulationPlatform ? ` / ${e(i.formulationPlatform)}` : ''}`),
    row('Formulation type / packaging', `${e(i.formulationType)}${i.packaging ? ` / ${e(i.packaging)}` : ''}`),
