    ...
    <>
      ...
      {open && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label={t('agro.aiChat.title')}
        >
          <div className="flex items-center justify-between gap-2 border-b border-border px-4 py-3">
            <div>
              <p className="text-[14px] font-semibold text-foreground">{t('agro.aiChat.title')}</p>
              <p className="text-[12px] text-fg-3">{t('agro.aiChat.subtitle')}</p>
            </div>
          </div>
          ...
          <div ref={listRef} className="flex-1 overflow-y-auto px-4 py-3" aria-live="polite" aria-atomic="false">
            {messages.length === 0 && (
              <div className="flex flex-col gap-2 pt-1">
                {(dictionaries[locale].agro.aiChat.chips as readonly string[]).map((chip) => (
                  <button
                    key={chip}
                    type="button"
                    onClick={() => send(chip)}
          ...
          >
            <input
              ...
              placeholder={t('agro.aiChat.placeholder')}
              maxLength={1000}
              aria-label={t('agro.aiChat.placeholder')}
