...
export function inquiriesToCsv(rows: InquiryRow[]): string {
  const header =
    'tier,score,category,business_type,project_stage,timeline,quantity,annual_volume,role,formulation_platform,formulation_type,customization,packaging,compliance,docs,budget,nda,company,website,country,target_market,email,whatsapp,requirements,file,status,locale,created_at'
